...
PRD `docs/prd/PRD_poc-server-refactor.md`의 **WI-1**. "동시 10통"이 미검증 추측값(`config.py`
주석 명시)이므로, **실제로 몇 통까지 안정적으로 처리하는지**를 외부 비용 없이 실측한다.

## 무엇을 재는가

서버의 CPU 병목은 **통화마다 도는 Local VAD(Silero) 추론 + 오디오 처리가 단일 asyncio
이벤트루프(`--workers 1`)에 쌓이는 것**이다(`local_vad.py`가 `to_thread` 없이 동기 실행).
...
`RELAY_LOAD_TEST_MODE=1`이면 (`src/config.py`):
